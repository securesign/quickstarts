...
```sh
export TUF_URL=$(oc get route tuf -n rhtas-gateway-api -o jsonpath='{.spec.host}')
export REKOR_URL=$(oc get route rekor -n rhtas-gateway-api -o jsonpath='{.spec.host}')
export FULCIO_URL=$(oc get route fulcio -n rhtas-gateway-api -o jsonpath='{.spec.host}')
```
...
```sh
curl -H "Authorization: Bearer $TOKEN" $FULCIO_UR/api/v2/trustBundle -i
...
```sh
curl -H "Authorization: Bearer $TOKEN" -X POST $FULCIO_UR/api/v2/signingCert -i
# HTTP/1.1 403 Forbidden
```
...
```sh
while :; do curl --write-out '%{http_code}\n' --silent --output /dev/null -H "Authorization: Bearer $TOKEN" $FULCIO_UR/api/v2/trustBundle | grep -E --color "\b(429)\b|$"; sleep 1; done
```
...
#sign image
HTTP_PROXY=127.0.0.1:9090 HTTPS_PROXY=127.0.0.1:9090 NO_PROXY=oauth2.sigstore.dev,ttl.sh cosign sign --fulcio-url=$FULCIO_URL --rekor-url=$REKOR_URL --certificate-oidc-issuer="https://oauth2.sigstore.dev/auth" -y $IMAGE_NAME
```
